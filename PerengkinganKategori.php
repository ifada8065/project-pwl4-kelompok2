  ...
  mysqli_query("TRUNCATE TABLE ranging");
  $query = mysqli_query("select * from bobot where kategori='".$_SESSION['kategori']."'");
  $r = mysqli_fetch_array($query);
  $bobot = array($r['c1'], $r['c2'], $r['c3'], $r['c4'], $r['c5']);

  //Buat fungsi tampilkan nama
  function getNama($id){
    $q =mysqli_query("SELECT * from kriteria where kategori='".$_SESSION['kategori']."' and id = '$id'");
    $d = mysqli_fetch_array($q);
    return $d['tipe_laptop'];
  }
//Lakukan Normalisasi dengan rumus pada langkah 2
  //Cari Max atau min dari tiap kolom Matrik
  $crMax = mysqli_query("SELECT max(c1) as maxK1,
            max(c2) as maxK2,
            max(c3) as maxK3,
            max(c4) as maxK4
      FROM kriteria");
  $max = mysqli_fetch_array($crMax);
  $crMin = mysqli_query("SELECT min(c5) as minK5
      FROM kriteria");
    $min = mysqli_fetch_array($crMin);

    $sql3 = mysqli_query("SELECT * FROM kriteria where kategori = '".$_SESSION['kategori']."'");
...
while ($dt3 = mysqli_fetch_array($sql3)) {
  ...
  mysqli_query("insert into ranging values ('$tampilnama','$Nilai')");
}
  ...
  <table id="a" class=" table table-striped  table-bordered table-hover" >
                                  <thead>
                                    ...
                                    <tbody>
                                            ...
                                            $kriteria1 = mysqli_query("select * from ranging order by nilai desc");
                                            $q=0;
